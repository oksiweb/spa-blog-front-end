...
function Search() {
  ...
  function searchKeyPressHandler(e) {
    if (e.keyCode == 27) {
      appDispatch({ type: IS_SEARCH_CLOSE })
    }
  }
    ...
    <>
      <div className="search-overlay-top shadow-sm">
        <div className="container container--narrow">
          ...
          <input value={state.searchTerm} onChange={handleInput} autoFocus type="text" autoComplete="off" id="live-search-field" className="live-search-field" placeholder="What are you interested in?" />
          <span onClick={() => appDispatch({ type: IS_SEARCH_CLOSE })} className="close-live-search">
            ...
            {Boolean(state.results.length) && (
              <div className="list-group shadow-sm">
                ...
                {state.results.map(post => {
                  return (<Post onClick={() => appDispatch({type: IS_SEARCH_CLOSE})}
